...
public abstract class PathNavigator {
   ...
   public Path getPathToEntity(Entity entityIn, int distance) {
      return this.func_225464_a(ImmutableSet.of(new BlockPos(entityIn)), 16, true, distance);
   }
   ...
   protected void pathFollow() {
      ...
      // Forge: fix MC-94054
      if (Math.abs(this.entity.getPosX() - (vec3d1.x + ((this.entity.getWidth() + 1) / 2D))) < (double)this.maxDistanceToWaypoint && Math.abs(this.entity.getPosZ() - (vec3d1.z + ((this.entity.getWidth() + 1) / 2D))) < (double)this.maxDistanceToWaypoint && Math.abs(this.entity.getPosY() - vec3d1.y) < 1.0D) {
         this.currentPath.setCurrentPathIndex(this.currentPath.getCurrentPathIndex() + 1);
      }

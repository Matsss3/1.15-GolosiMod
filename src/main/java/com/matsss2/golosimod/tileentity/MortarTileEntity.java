package com.matsss2.golosimod.tileentity;
...
public class MortarTileEntity extends TileEntity implements ITickableTileEntity, INamedContainerProvider {

	private ITextComponent customName;
	private GolosiItemHandler inventory;

	public MortarTileEntity(TileEntityType<?> tileEntityTypeIn) {
		super(tileEntityTypeIn);

		this.inventory = new GolosiItemHandler(2);
	}

	public MortarTileEntity() {
		this(ModTileEntityTypes.MORTAR.get());
	}

	@Override
	public Container createMenu(final int windowID, final PlayerInventory playerInv, final PlayerEntity playerIn) {
		return new MortarContainer(windowID, playerInv, this);
	}
	...
	public ITextComponent getDefaultName() {
		return new TranslationTextComponent("container." + GolosiMod.MOD_ID + ".mortar");
	}
	...
	private MortarRecipe getRecipe(ItemStack stack) {
		...
		Set<IRecipe<?>> recipes = findRecipesByType(RecipeSerializerInit.MORTAR_TYPE, this.world);
		for (IRecipe<?> iRecipe : recipes) {
			MortarRecipe recipe = (MortarRecipe) iRecipe;
	...
	public GolosiItemHandler getInventory() {
